...
function editRow(e) {
    ...
    for (var i = 0; i < schemaAr.length; i++) {
        console.log(schemaAr[i]);
        if(/data-timestamp="1"/.test(schemaAr[i])){
            var s=  arToTR[i].substr(0,arToTR[i].length-5);
            console.log(s);
            html += "<td>" + schemaAr[i].replace(/VAL/, s) + "</td>";
        }else{
...
function createEditAr(schema) {
    ...
    var tmpDate = '<input id="NAME" data-timestamp="1" name="NAME" value="VAL" type="datetime-local" placeholder="NAME">';
    ...
    var disabled = '<input id="NAME" name="NAME" value="VAL"  disabled class="disabled">';
    var s = '';
    var ar = [];
    for (var i = 0; i < schema.length; i++) {
        ...
        if(/timestamp/.test(schema[i].Type)){
            s = tmpDate.replace(/NAME/g, schema[i].Field);
            ar.push(s); 
        }
        if (/int/.test(schema[i].Type) && !/tinyint/.test(schema[i].Type) && !(/bigint/.test(schema[i].Type)) || /double/.test(schema[i].Type)) {
...
function getSchema(table) {
    $.ajax({
        type: 'GET',
        url: 'api/getSchemaTable/?table=' + table,
        error: function (xhr, str) {
            console.log('Возникла ошибка: ' + str);
        },
        success: function (xhr, str) {
